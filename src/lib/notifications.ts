...
/** الصلاة على النبي ﷺ */
export const SALAWAT_URL = "/media/salawat.mp3";
...
export type NotificationPrefs = {
  ...
  /** دقائق بعد الفجر لأذكار الصباح */
  morningOffset: number;
  /** دقائق بعد العصر لأذكار المساء */
  eveningOffset: number;
  /** عدد مرات الصلاة على النبي ﷺ يوميًا */
  salawatCount: number;
};
...
export function savePrefs(prefs: NotificationPrefs) {
  ...
  window.dispatchEvent(new CustomEvent("emty-prefs-changed"));
}
...
/** أوقات عشوائية يوميًا بين ٧ صباحًا و١١ مساءً (العدد حسب الإعدادات) */
export function salawatTimesToday(): number[] {
  if (typeof window === "undefined") return [];
  const today = new Date().toDateString();
  const count = Math.max(1, Math.min(50, loadPrefs().salawatCount));
  try {
    const stored = JSON.parse(localStorage.getItem(SALAWAT_KEY) ?? "{}");
    if (stored.date === today && Array.isArray(stored.times) && stored.times.length === count)
      return stored.times as number[];
  } catch {
    /* ignore */
  }
  const base = new Date();
  ...
  while (mins.size < count) mins.add(Math.floor(Math.random() * span));
